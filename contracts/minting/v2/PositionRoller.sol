...
contract PositionRoller {
     ...
     */
    function rollFullyWithExpiration(IPositionV2 source, IPositionV2 target, uint40 expiration) public {
     ...
     */
    function findRepaymentAmount(IPositionV2 pos) public view returns (uint256) {
     ...
     */
    function roll(IPositionV2 source, uint256 repay, uint256 collWithdraw, IPositionV2 target, uint256 mint, uint256 collDeposit, uint40 expiration) public valid(source) valid(target) own(source) {
        ...
        if (mint > 0) {
            ...
            if (Ownable(address(target)).owner() != msg.sender || expiration != target.expiration()) {
                ...
                target = IPositionV2(IMintingHub(target.hub()).clone(msg.sender, address(target), collDeposit, mint, expiration));
            } else {
