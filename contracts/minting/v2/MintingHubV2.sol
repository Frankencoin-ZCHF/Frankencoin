...
contract MintingHubV2 {
    ...
    struct Challenge {
        ...
        IPositionV2 position; // the position that was challenged
    ...
    function openPosition(
    ...
    ) public returns (address) {
        {
            ...
            } catch Error(string memory /*reason*/) {} catch Panic(uint /*errorCode*/) {} catch (bytes memory /*lowLevelData*/) {}
        ...
        IPositionV2 pos = IPositionV2(
            POSITION_FACTORY.createNewPosition(msg.sender, address(zchf), _collateralAddress, _minCollateral, _mintingMaximum, _initPeriodSeconds, _expirationSeconds, _challengeSeconds, _riskPremium, _liqPrice, _reservePPM)
        );
    ...
    function clone(address owner, address parent, uint256 _initialCollateral, uint256 _initialMint, uint40 expiration) public validPos(parent) returns (address) {
        ...
        IPositionV2 child = IPositionV2(pos);
     ...
     */
    function challenge(address _positionAddr, uint256 _collateralAmount, uint256 minimumPrice) external validPos(_positionAddr) returns (uint256) {
        IPositionV2 position = IPositionV2(_positionAddr);
     ...
     */
    function expiredPurchasePrice(IPositionV2 pos) public view returns (uint256) {
